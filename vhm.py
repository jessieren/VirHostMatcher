...
if (options.virusFaDir is None or
		options.hostFaDir is None or
		options.hostTaxaFile is None) :
...
for currentFileName in virusFaList :
	if currentFileName.startswith('.') :
		continue
	sys.stdout.write("Step 1: counting kmers for virus " + currentFileName + "\n")
	for w in range(1, (kmax+1)) :
		currentFilePath = os.path.join(options.virusFaDir, currentFileName)
		currentKmerCountPath = os.path.join(kmerCountPath, currentFileName)
		...
		cmdKmerOut.wait()
	virusListFileWrite.write(currentFileName + " " + \
...
for currentFileName in hostFaList :
	if currentFileName.startswith('.') :
		continue
	sys.stdout.write("Step 1: counting kmers for host " + currentFileName + "\n")
	for w in range(1, (kmax+1)) :
		...
		cmdKmerOut = subprocess.Popen(cmdKmer, shell=True, \
																			...
																			stdout = subprocess.PIPE)
		cmdKmerOut.wait()
	hostListFileWrite.write(currentFileName + " " + \
												 currentKmerCountPath + " " +\
												 str(2) + "\n")
hostListFileWrite.close()
